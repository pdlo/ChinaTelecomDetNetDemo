...
//******************************************************
    ...
    action route(bit<48> dst_mac,bit<9> port){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        standard_metadata.egress_spec = port;
        hdr.ipv4.setValid();
        //TODO：填充ipv4地址并且修改以太网类型
        hdr.ethernet.ether_type=ETH_TYPE_IPV4;
        hdr.ipv4.src_ipv4=0x0A000001;
        hdr.ipv4.dst_ipv4=0x0A000002;
        hdr.ipv4.protocol=IP_PROTO_TUNNEL;

    }
    action c2s(bit<48> dst_mac,bit<9> port,bit<128> dst_ipv6){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        standard_metadata.egress_spec = port;
        hdr.ipv6.dst_ipv6=dst_ipv6;
    }
    action ipv6_simple_forward(bit<48> dst_mac,bit<9> port){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        standard_metadata.egress_spec = port;
    }
    action s2c(bit<48> dst_mac,bit<9> port,bit<128> src_ipv6){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        standard_metadata.egress_spec = port;
    ...
    action route_reverse(bit<48> dst_mac,bit<9> port){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        standard_metadata.egress_spec = port;
        hdr.ipv4.setInvalid();
