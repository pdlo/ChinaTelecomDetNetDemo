...
control MyIngress(inout headers hdr,
                  inout metadata meta,
                  inout standard_metadata_t standard_metadata){
    register<bit<32>>(50) byte_cnt_reg;  //三个计数器，总共二十条链路，
    register<bit<32>>(50) packet_cnt_reg;//每一条链路都是对应这组寄存器当中的一个小格子 //在入口端的寄存器只有在（MBCN）中N的入口处才会被使用
    ...
    apply{
        if(hdr.ipv4.isValid()){        //说明这是一个数据包
            ...
            if(hdr.srv6h.last_entry==6 || hdr.srv6h.last_entry==8){   ////   4对7对6，5对9对8说明这是一个走来回的探测包，来回走的探测包有讲究，要在四个地方抓数据
            ...
            }else if(hdr.srv6h.last_entry==3 || hdr.srv6h.last_entry==4){   //说明这是一个单向INT包
                ...
                if((hdr.srv6h.last_entry==4&&hdr.srv6h.segment_left!=4)||(hdr.srv6h.last_entry==3&&hdr.srv6h.segment_left!=3)){
                    hdr.probe_data.push_front(1);
                    hdr.probe_data[0].setValid(); 
                    swid_single.apply();
                    hdr.probe_header.num_probe_data=hdr.probe_header.num_probe_data+1;
    ...
    apply {    
        if(hdr.ipv4.isValid()&&hdr.srv6h.isValid()){
            if(hdr.srv6h.last_entry==hdr.srv6h.segment_left+1){
                ...
                if((hdr.srv6h.last_entry==3&&hdr.srv6h.segment_left!=0)||(hdr.srv6h.last_entry==4&&hdr.srv6h.segment_left!=0)){
                    hdr.probe_data.push_front(1);
                    hdr.probe_data[0].setValid(); 
