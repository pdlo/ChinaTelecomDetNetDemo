...
{
    Counter<bit<32>, bit<8>>(8, CounterType_t.PACKETS_AND_BYTES) ingress_counter;
    Counter<bit<32>, bit<8>>(8, CounterType_t.PACKETS_AND_BYTES) egress_counter;
//---------------------------------------丢包--------------------------------------------------- 
    action drop() {
        ig_intr_dprsr_md.drop_ctl = 1;
    }
//---------------------------------------入口统计-----------------------------------------------
    action ingress_count(bit<8> counter_index) {
        ingress_counter.count(counter_index);
    }
    table ingress_count_srv6ipv4{
        key = {
...
//---------------------------------------ipv4转发-----------------------------------------------  
    action ipv4_forward(bit<48> src_mac, bit<48> dst_mac, bit<9> port, bit<8> counter_index) {
        egress_counter.count(counter_index);
        // 出端口
...
//---------------------------------------IPv6插入-----------------------------------------------
    action ipv6_insert(bit<128> src_ipv6, bit<128> dst_ipv6){
        ...
        hdr.ipv6.payload_len = hdr.ipv4.totalLen;  //srv6首部长度+负载长度
...
//---------------------------------------ipv6转发-----------------------------------------------  
    action ipv6_forward(bit<48> src_mac, bit<48> dst_mac, bit<9> port, bit<8> counter_index) {
        egress_counter.count(counter_index);
        // 出端口
        ig_intr_tm_md.ucast_egress_port = port;
        // MAC层
    ...
    apply {
        if (hdr.arp.isValid()) {
            ...
            if (hdr.srv6h.last_entry == 0) {
                ...
                hdr.srv6h.hdr_ext_len = hdr.srv6h.hdr_ext_len + 2;
            }
