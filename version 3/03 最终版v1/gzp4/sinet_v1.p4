...
//******************************************************
    ...
    action route(bit<48> dst_mac,PortId_t port){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        ig_intr_tm_md.ucast_egress_port = port;
        hdr.ipv4.setValid();
        hdr.ethernet.ether_type=ETH_TYPE_IPV4;
        //TODO：填充ipv4内容并且修改以太网类型，不要忘记填写IPV4的proto字段为160
    }
    action c2s(bit<48> dst_mac,PortId_t port,bit<128> dst_ipv6){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        ig_intr_tm_md.ucast_egress_port = port;
        hdr.ipv6.dst_ipv6=dst_ipv6;
    }
    action ipv6_simple_forward(bit<48> dst_mac,PortId_t port){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        ig_intr_tm_md.ucast_egress_port = port;
    }
    action s2c(bit<48> dst_mac,PortId_t port,bit<128> src_ipv6){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        ig_intr_tm_md.ucast_egress_port = port;
    ...
    action route_reverse(bit<48> dst_mac,PortId_t port){
        hdr.ethernet.src_mac=hdr.ethernet.dst_mac;
        hdr.ethernet.dst_mac=dst_mac;
        ig_intr_tm_md.ucast_egress_port = port;
        hdr.ipv4.setInvalid();
