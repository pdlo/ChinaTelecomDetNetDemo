...
{
    Register<bit<16>, bit<32>>(32w4, 0) register_packet_cnt_ingress;
    RegisterAction<bit<16>, bit<32>, bit<16>>(register_packet_cnt_ingress) register_packet_cnt_ingress_add_action = {
    ...
    Register<bit<16>, bit<32>>(32w4, 0) register_packet_len_ingress;
    RegisterAction<bit<16>, bit<32>, bit<16>>(register_packet_len_ingress) register_packet_len_ingress_add_action = {
        void apply(inout bit<16> value, out bit<16> out_value) {
    ...
    Register<bit<16>, bit<32>>(32w4, 0) register_packet_cnt_egress;
    RegisterAction<bit<16>, bit<32>, bit<16>>(register_packet_cnt_egress) register_packet_cnt_egress_add_action = {
        void apply(inout bit<16> value, out bit<16> out_value) {
    ...
    Register<bit<16>, bit<32>>(32w4, 0) register_packet_len_egress;
    RegisterAction<bit<16>, bit<32>, bit<16>>(register_packet_len_egress) register_packet_len_egress_add_action = {
        void apply(inout bit<16> value, out bit<16> out_value) {
...
//--------------------------------------------apply-------------------------------------------------------
    apply {
        if (hdr.arp.isValid()) {
            ...
            if (hdr.srv6h.last_entry == 0) {
                // hdr.srv6h.last_entry = 0;
                hdr.srv6_list[0].setValid();
                hdr.srv6_list[0].segment_id = meta.s1;
            }
            else if (hdr.srv6h.last_entry == 1) {
                // hdr.srv6h.last_entry = 1;
                hdr.srv6_list[0].setValid();
                hdr.srv6_list[0].segment_id = meta.s1;
        ...
        bit<32> register_packet_ingress_idx = (bit<32>) ig_intr_md.ingress_port;
        bit<16> packet_cnt_ingress = register_packet_cnt_ingress_add_action.execute(register_packet_ingress_idx);
        bit<16> packet_len_ingress = register_packet_len_ingress_add_action.execute(register_packet_ingress_idx);
        bit<32> register_packet_egress_idx = (bit<32>) ig_intr_tm_md.ucast_egress_port;
        bit<16> packet_cnt_egress = register_packet_cnt_egress_add_action.execute(register_packet_egress_idx);
        bit<16> packet_len_egress = register_packet_len_egress_add_action.execute(register_packet_egress_idx);
        if (hdr.probe.data_cnt == 1) {
            hdr.probe_data[0].packet_cnt_ingress = packet_cnt_ingress;
            hdr.probe_data[0].packet_len_ingress = packet_len_ingress;
            hdr.probe_data[0].packet_cnt_egress = packet_cnt_egress;
            hdr.probe_data[0].packet_len_egress = packet_len_egress;
        }
        else if (hdr.probe.data_cnt == 2) {
            hdr.probe_data[1].packet_cnt_ingress = packet_cnt_ingress;
            hdr.probe_data[1].packet_len_ingress = packet_len_ingress;
            hdr.probe_data[1].packet_cnt_egress = packet_cnt_egress;
            hdr.probe_data[1].packet_len_egress = packet_len_egress;
        }
    }
}
